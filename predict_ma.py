...
"""Index predicting Many-to-Many LSTM model.
 ...
 $ python predict.py --data_path=C:/Users/Admin/Desktop/Demian-marketbreaker
...
import reader_ma as reader
import path_ma as path
...
flags.DEFINE_string("predict_term", 5,
                    "predict the day after 1 week: 5, 1 month: 20, 3 months: 65")
flags.DEFINE_string("index", 7,
                    "The serial number for parameter selection")
FLAGS = flags.FLAGS
BASIC = "basic"
CUDNN = "cudnn"
BLOCK = "block"
...
def model_fn(features, labels, mode, params):
    ...
    # regression을 위해 output node가 1인 linear dense-layer를 만든다.
    #softmax_w = tf.get_variable(
    #    "softmax_w", [hidden_size, 1], dtype=data_type())
    #softmax_b = tf.get_variable("softmax_b", [1], dtype=data_type())
    #logits = tf.matmul(output, softmax_w) + softmax_b

    tv = tf.trainable_variables()
    tf.summary.scalar("first_layer_weight", tv[0][0, 0])
    tf.summary.scalar("second_layer_weight", tv[2][0, 0])

    #tf.summary.scalar("first_weight", softmax_w[0, 0])
    # logits[batch_size, times_steps]에서 1, 10, 20번째 step만 추출
    #logits = tf.reshape(logits, [-1, num_steps])
    #a = []
    #a.append(logits[:, 0])
    #a.append(logits[:, 9])
    #a.append(logits[:, 19])
    ...
    #if predict_term == 5: logit = 30 *logit
    #if predict_term == 20: logit = 40 *logit
    #if predict_term == 65: logit = 50 *logit
    ...
    # evaluation을 위한 target value dimension 정리
    #targets = tf.reshape(labels, [batch_size, num_steps, output_size])

    # targets[batch_size, times_steps, output_size]에서 1, 10, 20번째 step만 추출
    #b = []
    #b.append(tf.slice(targets, [0,0,0], [batch_size, 1, 1]))
    #b.append(tf.slice(targets, [0,9,0], [batch_size, 1, 1]))
    #b.append(tf.slice(targets, [0,19,0], [batch_size, 1, 1]))
...
def create_estimator(config):
    ...
    model_dir = path.model_dir +  "/kospi200f_" + str(config.predict_term) + "_" + str(config.step_interval) + "_" + \
                str(config.num_steps) + "_" + str(config.learning_rate) + "_" + str(config.hidden_size) + "_" + str(config.batch_size)
...
def train(model, train_data, config):
    ...
    #inputs = np.array(inputs).reshape(len(inputs)-config.num_steps+1, config.num_steps, config.input_size)
    #targets = np.array(targets).reshape(len(inputs)-config.num_steps+1, config.num_steps, config.output_size)
...
def predict(model, predict_data, config, test_start_index):
    ...
    ma_today, date, today = reader.make_index_date(predict_data, config.predict_term, test_start_index, config.step_interval, config.num_steps)

    index_ma_std_df = pd.read_csv("index-ma-std.csv", encoding="ISO-8859-1")

    if config.predict_term == 5:
        ma_df = index_ma_std_df.values[test_start_index : -config.predict_term, 2]
    if config.predict_term == 20:
        ma_df = index_ma_std_df.values[test_start_index : -config.predict_term, 3]
    if config.predict_term == 65:
        ma_df = index_ma_std_df.values[test_start_index : -config.predict_term, 4]
    ...
    ma_pred = []
    profits = []
    for i in range(len(ma_today)):
        ma_real.append(target_values[i] + ma_df[i])
        ma_pred.append(pred_values[i] + ma_df[i])
        if target_values[i]*pred_values[i] > 0: profits.append(abs(ma_today[i] - ma_real[i]))
        else: profits.append(-abs(ma_today[i] - ma_real[i]))

    comp_results = {"1date" : today, "4real" : target_values, "3prediction" : pred_values, "5ma_today" : ma_today, "6ma_pred" : ma_pred, "7ma_real" : ma_real, "2pred_date" : date, "8loss_profits" : profits}
    result_file = path.result_dir + "/" + path.ver + "_" + str(config.predict_term) + "_" + str(config.step_interval) + "_" + str(config.num_steps) + "_" + \
                  str(config.learning_rate) + "_" + str(config.hidden_size) + "_" + \
...
def main(_):
    ...
    config = path.Config()
    #eval_config = Config()
    #eval_config.batch_size = 1 # test인 경우 batch_size 1

    # data_pathe에 있는 train, test파일을 읽어  data frame 생성
    raw_data = reader.raw_data(config, FLAGS.data_path)
    train_data, test_data, predict_data, test_start_index = raw_data
