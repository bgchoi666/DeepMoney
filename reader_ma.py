...
def raw_data(config, data_path=None):
  ...
  input_target_list.append(config.input_size + 3)
...
def make_index_date(test_data, predict_term, test_start_index, step_interval, num_steps):
  ...
  #예측 시점의 ma list
  if predict_term == 5:
    index = index_df.values[test_start_index: test_start_index + test_data_size, 2]
  if predict_term == 20:
    index = index_df.values[test_start_index: test_start_index + test_data_size, 3]
  if predict_term == 65:
    index = index_df.values[test_start_index: test_start_index + test_data_size, 4]
  #예측 대상일 list
