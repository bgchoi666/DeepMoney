...
import reader_gradual_train as reader
import path_gradual_train as path
...
def create_estimator(config):
    ...
    model_dir = path.model_dir +  "/" + path.market + "_" + str(config.predict_term) + "_" + str(config.step_interval) + "_" + \
    ...
    #기존 모델 재 사용 모드용 디레토리
    model_dir_cont = path.model_dir +  "/kospi200f_" + str(config.predict_term) + "_" + str(config.step_interval) + "_" + \
                str(config.num_steps) + "_" + str(config.rnn_mode) + "_" + str(config.hidden_size) + "_" + str(config.batch_size) + \
...
def predict(model, predict_data, config, test_start_index):
    ...
    index_today, date, today, std = reader.make_index_date(predict_data, config.predict_term, test_start_index, config.step_interval, config.num_steps)
...
def main(_):
    ...
    for i in range(len(config.grad_train_terms) - 2):
        ...
        today, date, index_pred, index_real, index_today, profits, pred_values, target_values, std = predict(M2M, predict_data, config, test_start_index)

        # add the prediction results to the total variables
        today_tot += today
        date_tot += list(date)
        index_pred_tot += index_pred
    ...
    accuracy, _, _, _ = calculate_recall_precision(target_values_tot, pred_values_tot)
    comp_results = {"date_base": today_tot, "real": target_values_tot, "prediction": pred_values_tot, "index_today": index_today_tot,
                    "index_real": index_real_tot, "index_pred": index_pred_tot, "date_pred": date_tot, "loss_profits": profits_tot,
                    "std": std_tot}
    result_file = path.result_dir + "/" + "gradual" + "_" + path.market + "_" + str(config.predict_term) + "_" + str(
        config.step_interval) + "_" + str(config.num_steps) + "_" + \
