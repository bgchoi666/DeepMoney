...
import reader_diff as reader
import path_diff as path
...
flags.DEFINE_integer("predict_term", 20,
                    "predict the day after 1 week: 5, 1 month: 20, 3 months: 65")
flags.DEFINE_integer("index", 7,
...
def predict(model, predict_data, config, test_start_index):
    ...
    comp_results = {"date" : today, "real" : target_values, "prediction" : pred_values, "index_today" : index_today, "index_real" : index_real, "pred_date" : date, "loss_profits" : profits}
...
def main(_):
    ...
    if config.read_mode == "raw" : data = reader.raw_data(config, FLAGS.data_path)
    else : data = reader.norm_data(config, FLAGS.data_path)
    train_data, test_data, predict_data, test_start_index = data
