...
"""Index predicting Many-to-Many LSTM model.
 ...
 - model_reset - True : if a model exists, remove it and recreate.
                 False : resuse aleady existing models
...
import reader_4inputs as reader
import path_4inputs as path
...
def create_estimator(config):
    ...
    model_dir = path.model_dir +  "/kospi200f_" + str(config.predict_term) + "_" + str(config.step_interval) + "_" + \
                ...
                "_" + str(config.input_size)
    #model directory reset이 true이면 기존 directory 삭제, false이면 기존 directory 재사용
...
def predict(model, predict_data, config, test_start_index):
    ...
    index_today, date, today = reader.make_index_date(predict_data, config.predict_term, test_start_index, config.step_interval, config.num_steps)
    ...
    for i in range(len(index_today)):
        index_real.append(target_values[i] + index_df[i])
        index_pred.append(pred_values[i] + index_df[i])
    ...
    comp_results = {"date" : today, "real" : target_values, "prediction" : pred_values, "index1" : index_today, "index2" : index_real,
                    "pred_date" : date, "loss_profits" : profits}
