...
import reader_std as reader
import path_std as path
...
def predict(model, predict_data, config, test_start_index):
    ...
    std_today, date, today = reader.make_index_date(predict_data, config.predict_term, test_start_index, config.step_interval, config.num_steps)
    ...
    profits = []
    for i in range(len(std_today)):
        if (std_today[i] - target_values[i])*(std_today[i] - pred_values[i]) > 0: profits.append(abs(std_today[i] - target_values[i]))
        else: profits.append(-abs(std_today[i] - target_values[i]))

    comp_results = {"date" : today, "real" : target_values, "prediction" : pred_values, "std_index" : std_today, "pred_date" : date, "loss_profits" : profits}
